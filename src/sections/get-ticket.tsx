...
function ParticleBackground() {
	...
	useEffect(() => {
		...
		const ground = Bodies.rectangle(sceneRef.current.clientWidth / 2, 850, sceneRef.current.clientWidth, 100, {
			isStatic: true,
		...
		const handleMouseMove = (event: MouseEvent) => {
			...
			squares.forEach((square) => {
				const distance = Math.sqrt(
					Math.pow(square.position.x - mousePos.x, 2) +
					Math.pow(square.position.y - mousePos.y, 2)
				)

				if (distance < 150) {
					const forceMultiplier = 0.07 * (150 - distance) / 150
					const angle = Math.atan2(
						square.position.y - mousePos.y,
						square.position.x - mousePos.x
					)
	...
	return (
		<section className='relative h-[800px] bg-pallet-b-foreground-primary flex items-center justify-center mt-44 overflow-hidden'>
			...
			>
				...
				OBTENER TICKET
				<span className='px-4 py-2 text-sm bg-black/40'>Próximamente</span>
			</Button>
		</section>
	)
}
