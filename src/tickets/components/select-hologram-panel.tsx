...
export const SelectHologramPanel = ({
	...
	const getHologramTooltip = (value: string, disabled: boolean): { content: React.ReactNode, className: string, arrowClassName: string } => {
			...
			return {
				content: (
					<div className="flex items-center gap-2">
						...
						<span className="text-xs font-medium uppercase tracking-wide">TIER {tierNumber}+</span>
					</div>
				),
		...
		if (value.startsWith('academia')) {
			const tierName = value.includes('mensual') ? 'MENSUAL' :
				value.includes('trimestral') ? 'TRIMESTRAL' :
					value.includes('anual') ? 'ANUAL' : 'LIFETIME'
			return {
				content: (
					<div className="flex items-center gap-2">
						<WhiteMidudevLogo className="w-4 h-4" />
						<span className="text-xs font-medium uppercase tracking-wide">ACADEMIA {tierName}</span>
					</div>
				),
	...
	const holograms = [
		{ value: STANDARD_HOLOGRAM, imageIndex: 1, disabled: false },
		...TWITCH_HOLOGRAMS.slice(0, 3).map((label, index) => ({
			value: label,
			imageIndex: index + 2, // 2, 3, 4 for twitch-1, twitch-2, twitch-3
			disabled: !twitchTier || Number(twitchTier) < (index + 1)
			// disabled: false
		})),
		...ACADEMIA_HOLOGRAMS.slice(0, 3).map((label, index) => ({
			value: label,
			imageIndex: 5 + index, // 5, 6, 7 for first 3 academia holograms
			...
			<div className='bg-palette-border-foreground rounded-lg p-4'>
				<div className='grid grid-cols-6 gap-1'>
					{holograms.map((hologram) => {
						...
						const { content: tooltipContent, className: tooltipClassName, arrowClassName } = getHologramTooltip(hologram.value, hologram.disabled)

						if (hologram.disabled) {
							return (
								<Tooltip key={hologram.value} text={tooltipContent} tooltipPosition='top' tooltipClassName={tooltipClassName} arrowClassName={arrowClassName}>
									<div className="relative">
										<Button
											variant="ghost"
											size="small"
											className="aspect-square p-2 flex items-center justify-center opacity-50 cursor-not-allowed"
											disabled={true}
										>
											{hologram.value === STANDARD_HOLOGRAM ? (
												<div className='relative size-6 overflow-hidden border rounded-full border-palette-ghost after:h-px after:w-full after:-rotate-45 after:bg-palette-ghost after:absolute after:top-1/2 after:left-1/2 after:-translate-x-1/2 after:-translate-y-1/2'></div>
											) : (
												<img
						...
						return (
							<Tooltip key={hologram.value} text={tooltipContent} tooltipPosition='top' tooltipClassName={tooltipClassName} arrowClassName={arrowClassName}>
								<Button
									...
									size="small"
									className={cn('aspect-square p-2 flex items-center justify-center', isSelected && 'bg-palette-ghost/50 scale-[0.8]')}
									onClick={async () => await handleChangeHologramAndSave(hologram.value as HologramOption)}
								>
									{hologram.value === STANDARD_HOLOGRAM ? (
										<div className={cn('relative size-[1.8rem] overflow-hidden border rounded-full border-palette-ghost after:h-px after:w-full after:-rotate-45 after:bg-palette-ghost after:absolute after:top-1/2 after:left-1/2 after:-translate-x-1/2 after:-translate-y-1/2 transition-transform duration-300', isSelected && 'scale-[1.2] border-2 border-palette-default')}></div>
									) : (
										<img
											src={`/tickets/holograms/${hologram.imageIndex}.png`}
											alt={`Holograma ${hologram.value}`}
											className={cn('object-cover rounded-full transition-transform duration-300', isSelected && 'border-2 border-palette-default scale-[1.2]')}
											width='28'
											height='28'
										/>
									)}
