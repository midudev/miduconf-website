  ...
  return (
    <Layout meta={metadata}>
      <div aria-disabled className='w-[732px] -mb-[366px] relative -left-[200vw]'>
        <div id='ticket-2025' className='border-[16px] border-transparent'>
      ...
      >
        <div>
          ...
          <div className='flex flex-col items-center w-full px-8 mt-16 mb-16 gap-x-10 gap-y-4 lg:mb-0 lg:mt-4 md:flex-row'>
            ...
            <Button as='a' className='ml-0 lg:ml-auto' onClick={handleLogout} href='/'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='24'
                height='24'
      ...
      <Modal isOpen={isShowAchievementModal} onClose={() => setIsShowAchievementModal(false)}>
        ...
        <p className='px-4 py-2 my-4 text-center border rounded-lg bg-pallet-primary/20 border-pallet-primary/20 text-pallet-secondary text-balance'>
          ...
          <a href='https://midu.dev/usuario' target='_blank' className='underline' rel='noreferrer'>
            midu.dev
          </a>{' '}
          tu nuevo logro!
        </p>
...
const handleCreateTicketImage = async ({
  ...
  const dataURL = await toJpeg(document.getElementById('ticket-2025'), {
    quality: 0.8
  })
  ...
  const { error: errorStorage } = await supabase.storage.from('tickets').upload(filename, file, {
    cacheControl: '3600',
    upsert: true
  })
...
const useTicketSave = ({ buttonStatus, isMaterialChange }) => {
  ...
  useEffect(() => {
    toJpeg(document.getElementById('ticket-2025'), {
      quality: 0.8
